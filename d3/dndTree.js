...
function click(d) {
	...
	Show Tipsy for more Information contained in JSON file
	Must be called when node expands.
...
function update(source) {
	...
	nodeEnter.append("circle")
		...
		.style("fill", function(d) {
			...
			// fill circle based on 'type' of param
			if( typeof(d.type) != "undefined" && d.type == "TTP" ){
				return "#A901DB";
			}else if( typeof(d.type) != "undefined" && d.type == "ThreatActor" ){
				return "#B40404";
			}else if( typeof(d.type) != "undefined" && d.type == "Malware" ){
				return "#B18904";
			}else if( typeof(d.type) != "undefined" && d.type == "AttackPatterns" ){
				return "#FF8000";
			}else if( typeof(d.type) != "undefined" && d.type == "Report" ){
				return "#2E64FE";
			}else{
...
// format search data for semantic-ui search module
function format_search_data(d) {
    if (d.children)
        d.children.forEach(format_search_data);
    else if (d._children)
        d._children.forEach(format_search_data);
        
    tempAr = new Array();
    tempAr["title"] = d.name;    
    search_data.push(tempAr);
}

// collapse all not found nodes
